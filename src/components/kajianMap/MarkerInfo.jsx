...
export const MarkerInfo = ({ group, location, showAllInfo }) => {
      ...
      <>
        {group.map((info, i) => {
          return (
            <div
              key={i}
              className={`marker-info relative  min-w-[300px] flex flex-col mb-2 text-sm text-gray-800 bg-[#ffe7be] shadow-[inset_0_0_20px_-8px_#000] rounded-lg overflow-hidden ${
  ...
  return (
    <div
      className={`marker-info min-w-[300px] flex flex-col text-sm text-gray-800 bg-[#ffe7be] shadow-[inset_0_0_20px_-8px_#000] rounded-lg transition-opacity duration-300 overflow-hidden ${
        showAllInfo ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible"
      }`}
    >
