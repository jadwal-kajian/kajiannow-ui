...
function Row({ icon, title, desc }) {
  return (
    <div className="flex items-start gap-3 p-2.5 rounded-2xl bg-surface-2 text-left">
      <span className="w-8 h-8 shrink-0 rounded-full bg-accent text-accent-ink flex items-center justify-center">
        <FontAwesomeIcon icon={icon} className="text-xs" />
      </span>
      <div className="flex-1 min-w-0">
        <div className="font-semibold text-[13px] text-ink leading-tight">{title}</div>
        <div className="text-[11px] text-ink-dim leading-snug mt-0.5">{desc}</div>
      </div>
    </div>
  );
}
...
function Section({ children }) {
  return (
    <div className="text-left text-[11px] font-bold uppercase tracking-wide text-accent mt-2 px-1">
      {children}
    </div>
  );
}
  ...
  return (
    <div className={MODAL_SHELL}>
      ...
      <div className="px-4 pb-4 max-h-[64vh] overflow-y-auto flex flex-col gap-2 text-base">
        <p className="text-[12px] text-ink text-left leading-snug px-1">
        ...
        <div className="flex flex-col gap-1.5 p-2.5 rounded-2xl bg-surface-2 text-left text-[12px] text-ink">
